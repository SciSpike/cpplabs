  ...
  case 'f':
    {
      ...
      cin >> nameBuffer;
      cout << "\n";
      list<pair<string,string> > result = book.find(
	    [nameBuffer] (const pair<string,string>& p) {
	      return p.first.find(nameBuffer) != string::npos || p.second.find(nameBuffer) != string::npos;
	    }
      );
      if ( result.empty() ) {
